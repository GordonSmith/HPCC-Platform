...
void dbglog(const std::string str)
{
    ...
    hpcc_test_string_set(&msg, str.c_str());
...
// Helper Functions
// void hpcc_test_list_u32_free(hpcc_test_list_u32_t *ptr);
// void hpcc_test_string_set(hpcc_test_string_t *ret, const char*s);
...
void hpcc_test_list_test_string(hpcc_test_list_string_t *ret)
{
    ...
    for (size_t i = 0; i < ret->len; ++i)
    {
        std::string str = "test-";
        str += std::to_string(i);
        hpcc_test_string_dup(&ret->ptr[i], str.c_str());
    }
}
...
void hpcc_test_list_string_test_string(hpcc_test_list_string_t *a, hpcc_test_list_string_t *ret)
{
    ...
    for (size_t i = 0; i < ret->len; ++i)
    {
        hpcc_test_string_dup(&ret->ptr[ret->len - i - 1], std::string((const char *)v1[i].ptr, v1[i].len).c_str());
    }
...
void hpcc_test_list_test_list(hpcc_test_list_list_string_t *ret)
{
    ...
    {
        ret->ptr->len = 4;
        ret->ptr->ptr = (hpcc_test_string_t *)malloc(ret->ptr->len * sizeof(hpcc_test_string_t));
        for (size_t i = 0; i < ret->ptr->len; ++i)
        {
            std::string str = "test-";
            str += std::to_string(i);
            hpcc_test_string_dup(&ret->ptr->ptr[i], str.c_str());
        }
    }
}
