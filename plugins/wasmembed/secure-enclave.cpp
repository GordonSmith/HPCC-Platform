...
public:
    ...
    virtual void importFunction(size32_t lenChars, const char *qualifiedName) override
    {
        ...
        if (!wasmEngine->hasInstance(wasmName))
        {
            ...
            Owned<IFile> file = createIFile(fullPath.c_str());
            Owned<IFileIO> fileIO = file->open(IFOread);
            if (!fileIO)
                throw makeStringExceptionV(0, "Failed to open %s", fullPath.c_str());

            MemoryBuffer mb;
            size32_t count = read(fileIO, 0, (size32_t)-1, mb);
            wasmEngine->registerInstance(wasmName, mb.toByteArray());
        }
    }
