...
import { CreateDFUQueryStore, GetLF } from "src/ESPLogicalFile";
import { formatCost } from "src/Session";
import { convertedSize, formatDate, formatDecimal, formatNum } from "src/Utility";
...
export const Files: React.FunctionComponent<FilesProps> = ({
    ...
    const currentTime = React.useCallback((timestamp: string | number | Date) => {
        ...
        return uiState.isUTC ? formatDate(tsString, true) : formatDate(tsString, false);
    }, [uiState.isUTC]);
    //  Grid ---
    ...
    const columns = React.useMemo((): FluentColumns => {
        return {
            ...
            Name: {
                ...
                width: 180,
                formatter: (name, row) => {
                    const file = GetLF(row.NodeGroup, name, row);
                    if (row.__hpcc_isDir) {
                        return name;
                    }
            ...
            Records: {
                ...
                formatter: (value, row) => {
                    return formatNum(row.IntRecordCount);
                },
            ...
            FileSize: {
                label: nlsHPCC.Size,
                formatter: (value, row) => {
                    return convertedSize(row.IntSize);
                },
            ...
            CompressedFileSizeString: {
                ...
                formatter: (value, row) => {
                    return convertedSize(row.CompressedFileSize);
                },
            ...
            MinSkew: {
                label: nlsHPCC.MinSkew, width: 60, formatter: (value, row) => value ? `${formatDecimal(value / 100)}%` : ""
            },
            MaxSkew: {
                label: nlsHPCC.MaxSkew, width: 60, formatter: (value, row) => value ? `${formatDecimal(value / 100)}%` : ""
            },
            Accessed: {
    ...
    const [DeleteConfirm, setShowDeleteConfirm] = useConfirm({
        ...
        onSubmit: React.useCallback(() => {
            DFUArrayAction(selection, "Delete")
