...
export const Scopes: React.FunctionComponent<ScopesProps> = ({
    ...
    React.useEffect(() => {
        const files = [];
        if (scope === ".") {
            dfuService.DFUFileView({ Scope: "" }).then(({ DFULogicalFiles }) => {
                const _scopes = DFULogicalFiles?.DFULogicalFile.filter(file => file.isDirectory && file.Directory !== ".") || [];

                dfuService.DFUFileView({ Scope: "." }).then(({ DFULogicalFiles }) => {
                    _scopes.forEach((scope, idx) => {
                        scope["__hpcc_id"] = scope.Directory + "_" + idx;
                        files.push(scope);
                    });
                    DFULogicalFiles?.DFULogicalFile.forEach((file, idx) => {
            ...
            dfuService.DFUFileView({ Scope: scope }).then(({ DFULogicalFiles }) => {
                const _scopes = DFULogicalFiles?.DFULogicalFile.filter(file => file.isDirectory && file.Directory !== ".") || [];
                const _files = DFULogicalFiles?.DFULogicalFile.filter(file => !file.isDirectory) || [];
                _scopes.forEach((scope, idx) => {
                    scope["__hpcc_id"] = scope.Directory + "_" + idx;
                    files.push(scope);
                });
    ...
    const { Grid, selection, refreshTable, copyButtons } = useFluentGrid({
        ...
        columns: {
            ...
            __hpcc_displayName: {
                ...
                formatter: (_, row) => {
                    let name = row.Name;
                    let url = "#/files/" + (row.NodeGroup ? row.NodeGroup + "/" : "") + name;
                    if (row.isDirectory) {
                        name = row.Directory;
                        let path = [window.location.hash.replace(/#\/scopes\/?/, ""), row.Directory].join("::");
