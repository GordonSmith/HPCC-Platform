...
export const JsonImportForm: React.FunctionComponent<JsonImportFormProps> = ({
    ...
    const { handleSubmit, control, register, reset } = useForm<JsonImportFormValues>({ defaultValues });
                ...
                <table className={`${componentStyles.twoColumnTable} ${componentStyles.selectionTable}`}>
                    ...
                    { selection.map((file, idx) => {
                        return <tr key={`File-${idx}`}>
                            ...
                            <td>
                                <Controller
                                    control={control} name={`selectedFiles.${idx}.TargetRowPath` as const} defaultValue="/"
                                    render={({
                                        field: { onChange, name: fieldName, value },
                                        fieldState: { error }
                                    }) => <TextField
