...
export const FixedImportForm: React.FunctionComponent<FixedImportFormProps> = ({
    ...
    const { handleSubmit, control, register, reset } = useForm<FixedImportFormValues>({ defaultValues });
                ...
                <table className={`${componentStyles.twoColumnTable} ${componentStyles.selectionTable}`}>
                    ...
                    { selection.map((file, idx) => {
                        return <tr key={`File-${idx}`}>
                            ...
                            <td>
                                <Controller
                                    control={control} name={`selectedFiles.${idx}.RecordSize` as const} defaultValue=""
                                    render={({
                                        field: { onChange, name: fieldName, value },
                                        fieldState: { error }
                                    }) => <TextField
