...
import {
    ...
    IDragOptions, IIconProps, IStackStyles, keyframes, mergeStyleSets,
...
export const FileListForm: React.FunctionComponent<FileListFormProps> = ({
    ...
    >
        ...
        <div className={componentStyles.body}>
            <Stack>
                <Controller
                    control={control} name="dropzone" defaultValue={""}
                    render={({
                ...
                />
                <Controller
                    control={control} name="machines" defaultValue={undefined}
                    render={({
                        field: { onChange, name: fieldName, value },
                        fieldState: { error }
                ...
                />
                <Controller
                    control={control} name="path" defaultValue={""}
                    render={({
                        field: { onChange, name: fieldName, value },
                        fieldState: { error }
                ...
                <table className={`${componentStyles.twoColumnTable} ${componentStyles.selectionTable}`}>
                    ...
                    { selection.map((file, idx) => {
                        return <tr key={`File-${idx}`}>
                            ...
                            <td>{file["name"]}</td>
                            <td>{`${(parseInt(file["size"], 10) / 1024).toPrecision(2)} kb`}</td>
                        </tr>;
                    }) }
                    </tbody>
                </table>
