...
interface AsyncDropdownProps {
    ...
    errorMessage?: string;
    onChange?: (event: React.SyntheticEvent<HTMLDivElement> | undefined, option?: IDropdownOption | IDropdownOption[], index?: number | null) => void;
    placeholder?: string;
    className?: string;
    fieldClass?: string;
}
...
const AsyncDropdown: React.FunctionComponent<AsyncDropdownProps> = ({
    ...
    React.useEffect(() => {
        if (multiSelect) {
            if (!selOptions.length) return;
            const keys = selectedKey ? selectedKey.split(valueSeparator) : [];
            const currentKeys = selectedItems.map(i => String(i.key));
            if (keys.length === currentKeys.length && keys.every((k, i) => k === currentKeys[i])) return;

            const items = keys.map(key => selOptions.find(opt => String(opt.key) === String(key)) ?? { key, text: String(key) });
            setSelectedItems(items);
            setSelectedKeys(items.map(i => i.key).join(valueSeparator));
        } else {
            let item;
            if (selectedItem?.key) {
    ...
    }, [multiSelect, selectedKey, selOptions, selectedItem, selectedItems, required, onChange, valueSeparator]);
    ...
    const handleChange = React.useCallback((_, data) => {
        if (multiSelect) {
            ...
            if (onChange) {
                onChange(undefined, selectedOpts, null);
            }
        } else {
            const selectedIndex = selOptions.findIndex(opt => opt.key === data.optionValue);
            if (selectedIndex >= 0) {
                const selectedOption = selOptions[selectedIndex];
    ...
    }, [multiSelect, selOptions, onChange]);

    const dropdownValue = multiSelect
        ? selectedItems.map(item => item.text).join(",")
        : selectedItem?.text || "";

    const dropdownSelectedOptions = multiSelect
        ? selectedItems.map(item => String(item.key))
...
export const TargetFolderTextField: React.FunctionComponent<TargetFolderTextFieldProps> = (props) => {
    ...
    return folders === undefined ? <Skeleton><SkeletonItem /></Skeleton> : <Field label={label} validationMessage={errorMessage} required={required}>
...
export const CloudContainerNameField: React.FunctionComponent<CloudContainerNameFieldProps> = (props) => {
    ...
    const { label, selectedKey, onChange, ...restProps } = { ...props };
    ...
    const selectedKeys = React.useMemo(() => {
        ...
        return selectedKey.split(",").map(k => k.trim()).filter(k => k !== "");
    }, [selectedKey]);

    const displayValue = React.useMemo(() => {
        if (!selectedKeys || selectedKeys.length === 0) return "";
        return selectedKeys
            .map(key => options?.find(o => o.key === key)?.text || key)
            .filter(text => text !== "")
            .join(",");
    ...
    const handleOptionSelect = React.useCallback((ev, data) => {
        ...
        const newValue = newSelectedKeys.join(",");
        onChange?.(newValue);
    }, [onChange]);
    ...
    return options === undefined ? <Skeleton><SkeletonItem /></Skeleton> : <Field label={label}>
        <ComboBoxBase {...restProps as any} freeform multiselect autoComplete="on" value={displayValue} selectedOptions={selectedKeys} onOptionSelect={handleOptionSelect} onInput={handleInput}>
            {options?.map(option => (
                <Option key={String(option.key)} value={String(option.key)} text={option.text}>
                    {option.text}
                </Option>
