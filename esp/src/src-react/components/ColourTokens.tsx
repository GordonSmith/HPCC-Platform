...
const colourTokens: [string, string][] = Object.entries(tokens)
    .filter(([key]) => key.startsWith("color"))
    .sort(([a], [b]) => a.localeCompare(b)) as [string, string][];

interface ColourTokensProps {
    show: boolean;
    onClose: () => void;
}

export const ColourTokens: React.FunctionComponent<ColourTokensProps> = ({ show, onClose }) => {
    ...
    return <Dialog open={show} modalType="modal" onOpenChange={onDialogOpenChange}>
        <DialogSurface className={styles.dialogSurface}>
            <DialogBody>
                <DialogTitle>{nlsHPCC.ColourTokens}</DialogTitle>
                <DialogContent>
                    <div className={styles.content}>
                        {colourTokens.map(([key, value]) => (
