...
import { Button, Tooltip, makeStyles, tokens, Text, Link, List, ListItem, Menu, MenuDivider, MenuGroup, MenuGroupHeader, MenuItem, MenuList, MenuPopover, MenuTrigger, InfoLabel, ToolbarButton } from "@fluentui/react-components";
...
const ActiveWorkunitMenu: React.FC<ActiveWorkunitMenuProps> = ({ wu, canUp, canDown, setPriority, moveTop, moveUp, moveDown, moveBottom }) => {
    return <Menu>
...
const ActiveWorkunit: React.FC<ActiveWorkunitProps> = ({ wu, idx, listLength, setPriority, moveTop, moveUp, moveDown, moveBottom, wuPause, wuResume }) => {
...
const ActiveWorkunitList: React.FC<ActiveWorkunitListProps> = ({
    list,
    setPriority,
    moveTop,
    moveUp,
...
const StatusDetails: React.FC<StatusDetailsProps> = ({ details }) => {
    const d = details ?? "";
    const lines = d.split(";").map(s => s.trim()).filter(Boolean);
    if (lines.length === 0) return null;
    return <div style={{ display: "grid", rowGap: 2 }}>
        {lines.map((line, i) => {
            if (i === 0) {
...
export const QueueCards: React.FC<QueueCardsProps> = ({
    refreshToken
}) => {
