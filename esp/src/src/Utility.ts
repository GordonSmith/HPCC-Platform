...
export function encodeXML(str) {
    ...
    // Use a single pass with a lookup map for better performance
...
export function format(labelTpl, obj) {
    // Handle escaped newlines once at the beginning
    labelTpl = labelTpl.split("\\n").join("\n");

    const lines = labelTpl.split("\n");
    const result: string[] = [];

    // Process lines in a single pass to avoid intermediate arrays
